...
class SpeechAnalysisService: NSObject, ObservableObject, SFSpeechRecognitionTaskDelegate {
    ...
    private func processResult(_ result: SFSpeechRecognitionResult) async {
        ...
        await MainActor.run {
            ...
            if wordCount == 0 {
                self.errorMessage = "No speech detected"  // Update error message if no speech
                self.analysisProbabilities[currentURL] = 0  // Set probability to 0
            } else {
...
extension SpeechAnalysisService: SFSpeechRecognizerDelegate {
    func speechRecognizer(_ speechRecognizer: SFSpeechRecognizer, availabilityDidChange available: Bool) {
        DispatchQueue.main.async {
            if !available {
                self.errorMessage = "Speech recognition became unavailable"  // Notify when speech recognition becomes unavailable
            } else {
